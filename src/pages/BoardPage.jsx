...
export default function BoardPage() {
  ...
  async function handleOnDragEnd(result) {
    ...
    const oldOrder = currentBoard.lists.flatMap(l => l.cards).find(c => c.id === cardId).order;
    let newOrder = 0;

    if(destinationId === sourceId) {
      if(destinationIndex === 0) {
        newOrder = destinationList.cards.length > 0 ? destinationList.cards[0].order / 2 : 1024;
      }
